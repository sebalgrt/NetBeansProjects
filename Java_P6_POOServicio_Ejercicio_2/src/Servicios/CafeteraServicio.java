...
public class CafeteraServicio {
    ...
    public void llenarCafetera() {
        nueva.setCantAct(nueva.getCapMax());
    }
    
    public void servirTaza() {
        System.out.print("Ingrese tamaño de la taza (cc): ");
        int tamaño = leer.nextInt();
        if (nueva.getCantAct() < tamaño) {
            System.out.println("No alcanza, solo se servirá " + nueva.getCantAct());
        }else {
            nueva.setCantAct(nueva.getCantAct() - tamaño);
        }
        System.out.println("Quedan " + nueva.getCantAct() + " cc de café");
    }
    ...
    public void agregarCafe() {
        int ing = 0;
        do {
            System.out.print("Ingrese cantidad de café: ");
            ing = leer.nextInt();
            if (ing > nueva.getCapMax()) {
                System.out.println("Exeso de café, la capacidad máxima es: " + nueva.getCapMax());
            }else {
                nueva.setCantAct(ing);
            }
        } while (ing > nueva.getCapMax());
    }
}
